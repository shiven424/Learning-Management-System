from flask import Blueprint, render_template, request, redirect, url_for, session
from grpc_client import grpc_client
import grpc
import lms_pb2
import logging
bp = Blueprint('auth', __name__)

@bp.route('/')
def home():
    return render_template('login.html')

@bp.route('/register', methods=['GET', 'POST'])
def register():
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']
        role = request.form['role']
        name = request.form.get('name', '')

        try:
            response = grpc_client.stub.Register(lms_pb2.RegisterRequest(username=username, password=password, role=role, name=name))
            
            if response.status == "Registration successful":
                return redirect(url_for('auth.home'))
            else:
                # In case the registration failed due to a user already existing
                return render_template('register.html', error="Registration Failed: User already exists"), 400
        
        except grpc.RpcError as e:
            # Handle gRPC errors properly
            error_message = grpc_client.handle_grpc_error(e)
            return render_template('register.html', error=error_message), 500  # Show the error message

    return render_template('register.html')

@bp.route('/login', methods=['GET', 'POST'])
def login():
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']
        
        try:
            logging.info(f"{grpc_client.leader_address}")
            response = grpc_client.stub.Login(lms_pb2.LoginRequest(username=username, password=password))
            
            if response.status == "Success":
                session['token'] = response.token
                session['role'] = response.role
                session['username'] = username
                session['logged_in'] = True
                return redirect(url_for('dashboard.dashboard'))
            else:
                # Return an error if login was unsuccessful
                return render_template('login.html', error="Incorrect username or password"), 401
        
        except grpc.RpcError as e:
            error_message = grpc_client.handle_grpc_error(e)
            return render_template('login.html', error=error_message), 500

    return render_template('login.html')

@bp.route('/logout')
def logout():
    token = session.pop('token', None)
    if token:
        try:
            grpc_client.stub.Logout(lms_pb2.LogoutRequest(token=token))
            session.clear()
        except grpc.RpcError as e:
            grpc_client.handle_grpc_error(e)
    return redirect(url_for('auth.home'))


def check_session():
    if 'token' not in session:
        return False
    return True
